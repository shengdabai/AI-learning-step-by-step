...
(function () {
  ...
  window.checkQuiz = function (containerId) {
    ...
    $$('.quiz-question-block', container).forEach(q => {
      ...
      const rightExp  = q.dataset.explanationRight || q.dataset.explanationRightZh || '';
      const wrongExp  = q.dataset.explanationWrong  || q.dataset.explanationWrongZh  || '';
      const rightZh   = q.dataset.explanationRightZh || '';
      const wrongZh   = q.dataset.explanationWrongZh  || '';

      if (!selected) {
        feedback.innerHTML = '<span class="lang-zh">请先选择一个答案！</span><span class="lang-en">Pick an answer first!</span>';
      ...
      if (selected.dataset.value === correct) {
        ...
        const expHtml = rightZh
          ? `<span class="lang-zh">${rightZh}</span><span class="lang-en">${rightExp}</span>`
          : '<strong>Exactly!</strong> ' + rightExp;
        feedback.innerHTML = expHtml;
        feedback.className = 'quiz-feedback show success';
      } else {
        selected.classList.add('incorrect');
        const correctBtn = $(`.quiz-option[data-value="${correct}"]`, q);
        if (correctBtn) correctBtn.classList.add('correct');
        const expHtml = wrongZh
          ? `<span class="lang-zh">${wrongZh}</span><span class="lang-en">${wrongExp}</span>`
          : '<strong>Not quite.</strong> ' + wrongExp;
        feedback.innerHTML = expHtml;
